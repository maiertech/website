import type { PostMetaType } from '@maiertech/sveltekit-helpers';

const meta: PostMetaType = {
	title: 'Exploring frequently used methods of d3-array',
	author: 'thilo',
	publishedDate: '2022-06-27',
	description:
		'Learn how to use d3-array methods like min, max, extent, range, and ticks for data visualization, with interactive StackBlitz examples.',
	tags: ['data-visualization', 'd3'],
	path: '/posts/exploring-frequently-used-methods-of-d3-array',
	filepath: 'src/routes/posts/(2022)/exploring-frequently-used-methods-of-d3-array/+page.svx'
};

export default meta;
